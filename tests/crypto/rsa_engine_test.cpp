...
int expect_file_round_trip(
    ...
    if (!files_equal(plaintext_path, decrypted_path)) {
        std::cerr << "RSA file comparison failed\n"
                  << "expected: " << plaintext_path << '\n'
                  << "actual:   " << decrypted_path << '\n';
        return 1;
    }

    std::cout << "RSA file comparison passed: " << plaintext_path.filename().string() << '\n'
...
int main() {
    ...
    const std::filesystem::path output_dir =
        std::filesystem::path(UMBRA_BINARY_DIR) / "test_outputs" / "crypto" / "rsa_engine";
    ...
    std::cout << "RSA engine file tests passed\n";

    // --- Signature tests ---

    std::cout << "\nRunning RSA signature tests...\n";
